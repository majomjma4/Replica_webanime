(() => {
  ...
  const menuItems = [
    ...
    { name: "PelÃ­culas", href: buildAppUrl("peliculas"), icon: "movie" },
  ...
  document.addEventListener("click", (event) => {
    const profileBtn = event.target.closest("[data-profile-trigger]");
    const guestMenu = document.querySelector("[data-guest-menu]");

    if (profileBtn) {
      if (profileBtn.dataset.bound === "1") return;
      if (isLoggedIn()) {
        window.location.href = buildAppUrl("user");
        return;
      }
      ...
      if (!guestMenu) return;
      const nextOpen = guestMenu.classList.contains("hidden");
      guestMenu.classList.toggle("hidden", !nextOpen);
      profileBtn.setAttribute("aria-expanded", nextOpen ? "true" : "false");
      return;
    }

    if (!guestMenu || guestMenu.classList.contains("hidden") || isLoggedIn()) return;
    if (guestMenu.contains(event.target)) return;
    guestMenu.classList.add("hidden");
    document.querySelectorAll("[data-profile-trigger]").forEach((btn) => btn.setAttribute("aria-expanded", "false"));
  });
