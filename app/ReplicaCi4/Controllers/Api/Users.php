...
final class Users extends BaseController
{
    public function handle(): \CodeIgniter\HTTP\ResponseInterface
    {
        ...
        if ($action === 'toggle_status') {
            ...
            if ($userId === $adminId) return $this->response->setJSON(['success' => false, 'error' => 'No puedes cambiar tu propio estado desde aquí']);
        ...
        if ($action === 'block') {
            ...
            if (!array_key_exists($penalty, $penaltyMap)) return $this->response->setJSON(['success' => false, 'error' => 'Debes elegir un tiempo de penalización válido']);
        ...
        if ($action === 'delete') {
            $userId = (int) ($data['id'] ?? 0);
            $adminId = (int) ($session->get('user_id') ?? 0);
            if ($userId === $adminId) return $this->response->setJSON(['success' => false, 'error' => 'No puedes eliminar tu propia cuenta desde aquí']);
            if ($userId <= 0) return $this->response->setJSON(['success' => false, 'error' => 'ID de usuario invalido']);
        ...
        if ($action === 'reset_password') {
            ...
            if ($userId <= 0) return $this->response->setJSON(['success' => false, 'error' => 'ID de usuario invalido']);
            if ($isProtectedPrimaryAdmin($dbConn, $userId)) return $this->response->setJSON(['success' => false, 'error' => 'No puedes cambiar la contraseña del administrador principal']);
            if (mb_strlen($newPassword) < 6) return $this->response->setJSON(['success' => false, 'error' => 'La contraseña debe tener al menos 6 caracteres']);
            ...
            try {
                ...
                if ($currentHash !== '' && password_verify($newPassword, $currentHash)) {
                    return $this->response->setJSON(['success' => false, 'error' => 'La nueva contraseña no puede ser igual a la actual']);
                }
        ...
        if ($action === 'toggle_admin') {
            ...
            try {
                ...
                if ($targetRoleId <= 0) return $this->response->setJSON(['success' => false, 'error' => 'No se encontró el rol solicitado']);
